...
class TphereCommand extends BaseCommand
{
    ...
    public function __construct()
    {
        ...
        $this->setUsage('/tphere <accept {player} | decline {player} | {player}>');
    }
    ...
    public function execute(CommandSender $sender, string $commandLabel, array $args): bool
    {
        if ($sender instanceof Player) {
            ...
            if ($args[0] === 'a' || $args[0] === 'accept') {
                ...
                } elseif (($player = $sender->getServer()->getPlayerExact($args[1])) instanceof Player) {
                ...
                if ($sender->hasPermission(MyPlotPermissions::RANK_EMERALD)) {
                    ...
                    $this->getPlugin()->sendInfo($player, "{$sender->getName()} wants you to teleport to them. Use /tphere accept {$sender->getName()} or /tphere decline {$sender->getName()}.");
                } else {
